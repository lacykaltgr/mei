...
class MEI(Gabor):
    def __init__(self, models=None, shape=(1, 28, 28), bias=0, scale=1, device='cpu'):
    ...
    def generate(self, neuron_query, **MEIParams):
        ...
        #cont, vals, lim_contrast = contrast_tuning(model, mei, device=self.device)
    ...
    def gradient_rf(self, neuron_query, **MEIParams):
        ...
        model = adj_model(self.models, neuron_query)
        init_process = MEIProcess(model, bias=self.bias, scale=self.scale, device=self.device, **MEIParams)
